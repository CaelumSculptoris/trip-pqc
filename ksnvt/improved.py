...
if __name__ == "__main__":
    action = input(f"{bcolors.BOLD}Choose action (encrypt/decrypt): ").strip().lower()
    ...
    if action == "encrypt":
        ...
        print(f"{bcolors.WARNING}Encryption key (hex, save this to decrypt): {key.hex()}{bcolors.ENDC}")

    elif action == "decrypt":
        try:
            key_hex = input(f"{bcolors.BOLD}Enter encryption key (hex): ").strip()
            key = bytes.fromhex(key_hex)
        except ValueError:
            print(f"{bcolors.FAIL}Invalid key format!{bcolors.ENDC}")
            exit(1)

        try:
            vec_input = input(f"{bcolors.BOLD}Enter encrypted vector as comma-separated floats: ")
            encrypted_vec = np.array([float(x) for x in vec_input.split(',')], dtype=np.float64)
        except Exception as e:
            print(f"{bcolors.FAIL}Invalid vector input: {e}{bcolors.ENDC}")
            exit(1)
        ...
        print(f"{bcolors.FAIL}Unknown action!{bcolors.ENDC}")